...
class SpacePointGraph(BaseTransform):
    ...
    of local point density — unlike a radius cutoff, whose degree grows
    unboundedly with local density. The resulting graph is directed (guaranteed
    in-degree k, not necessarily mutual nearest-neighbours), standard practice
    for point-cloud GNNs.

    This should eventually be computed once at dataset-processing time in
    pynuml.process.HitGraphProducer, mirroring how the 2D per-plane Delaunay
    edges are baked in there. For now it is (re)built here on every load.
    ...
    Args:
        ...
        chunk_size: Number of query rows to compare against all spacepoints
            at once. Crowded events can have tens of thousands of spacepoints,
            and computing all pairwise distances in one shot is quadratic in
            memory — chunking bounds peak memory to chunk_size * n instead,
