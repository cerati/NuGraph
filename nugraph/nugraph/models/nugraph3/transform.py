...
class Transform(BaseTransform):
    ...
    def forward(self, data: NuGraphData) -> NuGraphData:
        ...
        # fix edge shapes when single dimensional (happens in optical edges)
        for store in data.edge_stores:
            if hasattr(store, 'edge_index') and store.edge_index.dim() == 1:
                store.edge_index = store.edge_index.unsqueeze(1)
